...
static int twl_mmc_card_detect(int irq)
{
	...
	for (i = 0; i < ARRAY_SIZE(hsmmc); i++) {
		...
		r = (mmc->slots[0].switch_active_low ^ gpio_get_value_cansleep(mmc->slots[0].switch_pin));

		// wait for sd to power-up, if present : we need 
		// this with some of our devices (a35) detection
		// and mmc power logic implementation, applying
		// power to uSD card only once detected.
		if (r && mmc->slots[0].switch_active_low)
			msleep(50);
...
void __init twl4030_mmc_init(struct twl4030_hsmmc_info *controllers)
{
	...
	for (c = controllers; c->mmc; c++) {
		...
		if (gpio_is_valid(c->gpio_cd)) {
			...
			mmc->slots[0].switch_active_low = c->gpio_cd_active_low;
